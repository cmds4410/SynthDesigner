...
//                                     ) * SineWave().freq(50);
    ...
    LPF24 filter = LPF24().Q(2).cutoff( 16000 );
    
    Reverb verb = Reverb().wetLevel(0.4).dryLevel(1.0).roomSize(0.4).decayTime(2.0).roomShape(0.7).decayLPFCutoff(8000);
    
    Generator output = (( tone * env ) >> filter >> verb) * 0.3;
